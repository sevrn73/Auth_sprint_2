...
from flask import current_app, redirect, request, url_for, jsonify, Response
    ...
    def authorize(self) -> Response:
        return redirect(
            self.service.get_authorize_url(
                scope="login:email login:info", response_type="code", redirect_uri=self.get_callback_url(),
            )
        )
...
class GoogleSignIn(OAuthSignIn):
    ...
    def authorize(self) -> Response:
        return redirect(
            self.service.get_authorize_url(scope="email", response_type="code", redirect_uri=self.get_callback_url(),)
        )
